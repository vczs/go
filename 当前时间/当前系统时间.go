...
func case_1(){
	...
	fmt.Printf("当前时间：%v年%v月%v日%v时%v分%v秒",times.Year(),int(times.Month()),times.Day(),times.Hour(),times.Minute(),times.Second())
}
...
func case_2(){
	...
	fmt.Printf(times.Format("2006/01/02 15:04:05"))
}


func case_3(){
	times := time.Now()
	fmt.Println(times.Unix()) //获取时间戳 单位秒
	fmt.Println(times.UnixNano()) //获取时间戳 单位毫秒
}
